...
// STABILIZATION:
// - UI-only korunur
// - Avatar seçim mantığı SADECE Screen seviyesinde
// - Form içinde avatar render edilmez
// - Array alanları normalize edildi
// - Crash riskleri kaldırıldı
// - React.memo ile render optimizasyonu
...
type Props = {
  ...
  /** Avatar seçimi screen tarafından yapılır */
  onPressAvatar?: () => void;
};
...
function CorporateIndividualProfileInlineForm({
  ...
  const focusAreas = normalizeArray(value.focusAreas);
  const highlights = normalizeArray(value.highlights);

  function patch<K extends keyof IndividualDraft>(
    key: K,
    next: IndividualDraft[K]
  ) {
